...
### WU-33c6 — copy-on-write storage for `OwnedSourceRaster`, removing `FileBackSource`'s own per-frame full-buffer copy `todo` — written this session, not built

**Same-session follow-up (Session 82) to WU-33c4/ADR-099's own parked frame-rate finding — investigated in full this session rather than re-parked a second time. See `DECISIONS.md` ADR-100 for the full account, not repeated in full here.** Real evidence: Steve's own `build/Testing/Temporary/LastTest.log` run of `test_decklink_live_backsrc_demo` (Aug 31, 12:53 BST) shows `framesArrived=752` against only `framesPushed=114` into the 8-slot `CaptureFrameRing` (`io/decklink_input.hpp`'s own `kCaptureRingCapacity=8`) — roughly 15%. `FileBackSource::currentSourceRaster()` (WU-33c2a) was named there as the leading, unconfirmed suspect; confirmed this session, not merely re-asserted: at 720x576 it performs three fresh heap allocations and roughly 2.5 MB memcpy'd, unconditionally, on every single `processOne()` call (WU-33c3), for content that never changes after `create()`.

**Confirmed by grep across `src/` and `tests/`, not assumed: no code anywhere mutates an `OwnedSourceRaster` after construction** — the only write to its own `rgb` member happens once, inside `unpackSourceRaster()` (`core/pipeline.cpp`), before the object is ever copied or shared; every other reference (`view()`, both back-source producers' own accessors, `CaptureConsumer::processOne()`'s own `backSnapshot`) is read-only, `SourceRaster`'s own `r`/`g`/`b` fields being `const Sample*` with no non-const `view()` overload anywhere. This is what makes shared, reference-counted storage safe with no copy-on-write branch needed at all.

**Design: `OwnedSourceRaster`'s own `rgb` member becomes a `std::shared_ptr<video::RasterRGB>` (first use of `std::shared_ptr` anywhere in `src/`, confirmed by grep, named plainly).** Copying an `OwnedSourceRaster` becomes an atomic refcount increment instead of a three-vector deep copy. Extends `DECISIONS.md` ADR-094 — does not reopen it: `OwnedSourceRaster`'s existence, purpose, and external `view()` contract are all unchanged; only its own internal copy cost, which ADR-094 never settled either way, changes. See ADR-100 for the rejected alternatives (widening the shared `BackSourceCallback` signature itself; a virtual/type-erased interface).

**Files:** `src/core/resolve.hpp` (modified — `OwnedSourceRaster`'s `rgb` member/constructor, `view()`). `src/core/pipeline.cpp` (modified — `unpackSourceRaster()`'s five touched lines, mechanical `.` → `->` only). `tests/test_unpack_source_raster.cpp` (modified, doesn't count against the cap — two new checks alongside WU-33c1's own existing two). **Zero files touched in `io/`** — confirmed by grep: every current `OwnedSourceRaster` caller already treats it as an opaque returned value, so `FileBackSource`/`DeckLinkBackSource`/`CaptureConsumer` all get this unit's own benefit automatically, with no call-site change anywhere. Comfortably inside the 3-source-file cap (2 of 3 used).

**Accept — written this session, NONE of it built or run:** WU-33c1's own two existing checks stay green, unchanged in intent (bit-identical output vs. `runFrameBytes()`; a copy's own `view()` survives the original's destruction — now trivially true). Two new checks: (3) a copy's own `view().r`/`.g`/`.b` compare pointer-*equal* to the original's — direct, sandbox-checkable proof the buffer is no longer duplicated on copy; (4) two independently-constructed `OwnedSourceRaster`s compare pointer-*unequal* — rules out a broken single-shared-static-buffer implementation that would pass check 3 vacuously.

**Explicitly not claimed: any frame-rate/timing number, from this sandbox or the device bridge's own Linux VM.** Same discipline `DECISIONS.md` ADR-044 already established for WU-19b: a wall-clock measurement against real hardware (Steve's own UltraStudio capture pipeline) means nothing measured on a CPU with no relationship to it. What this unit proves, entirely in-repo: the copy is real and, after this change, gone — down from three heap allocations and ~2.5 MB memcpy'd to one atomic increment, per call. **Whether this was actually the cause of Steve's own reported frame-rate drop is his own job to confirm**, by rebuilding, rerunning `test_decklink_live_backsrc_demo`, and comparing `framesPushed`/`framesArrived` against the 114/752 baseline this unit's own evidence section already cites. If the ratio doesn't move, this unit's own value still stands on correctness/design merits alone — a real, needless per-frame cost is gone either way.

**Not built, not run, not confirmed, by this or any prior session.** `core/resolve.hpp`/`core/pipeline.cpp` are portable, DeckLink-independent scatter-core files — unlike every other WU-33c-lineage unit, this one is fully cloud-sandbox-and-device-bridge buildable; building and running `tests/test_unpack_source_raster.cpp` for real is this session's own next honest step if Steve wants it done now rather than merely scoped, since nothing here needs his real terminal.
