...
### WU-33b — wire `PipelineParams::backSrc` into `runFrame()`/`runFrameField()` `green`
...
concrete template to work from. Depends on WU-33a (done, `green`).

Re-derived directly against the real current `core/pipeline.cpp` this
session (Session 77), not assumed from this note's own account or from
ADR-091's WU-34c precedent (per `DECISIONS.md` ADR-093): six call sites
total, not three — plain and TagByFacing sibling at each of the three
dispatch points named above, exactly WU-34c's own "six call sites, not
three" finding (ADR-091) repeated for this field. `runThreaded()` itself
gains one new trailing parameter (`const SourceRaster* backSrc`, after
`shadingGrid`) so its own two `generateFragmentsRowRange()`/
`generateFragmentsRowRangeTagByFacing()` calls can pass it through;
`runFrame()`'s two `runThreaded()` call sites (the `params.pool !=
nullptr` branch and the per-call `ThreadPool` branch) both pass
`params.backSrc` as the new trailing argument.

Unlike WU-34c's `shadingGrid`, `backSrc` needs no per-call construction
step: `params.backSrc` is already the exact `const SourceRaster*` type
every one of WU-33a's six entry points takes, so this unit's own job is
pure forwarding, not building-then-forwarding — `runFrame()`'s and
`runFrameField()`'s own PASS-1 call sites pass `params.backSrc` straight
through, unconditionally, at both the plain and TagByFacing branch of
each dispatch point (raster selection and tag selection are independent
gates at the `core/binner.hpp` level, ADR-092 — this file does not gate
`backSrc`'s propagation on `kBufferMode`/`frontTag != backTag` the way it
gates which of the two sibling functions gets called in the first place).

**Files:** `src/core/resolve.hpp` (`PipelineParams` gains `backSrc`, doc
comment following the established `pool`/`weightOut`/`kBufferMode`/
`lightingScene` "optional, caller-owned, default-off, zero-cost-when-
absent" pattern); `src/core/pipeline.cpp` (`runThreaded()` gains the new
trailing `backSrc` parameter; all six PASS-1 call sites, plus its own two
`runThreaded()` call sites, pass `params.backSrc`/`backSrc` through;
file-header comment extended); `tests/test_pipeline_backsrc.cpp` (new —
see below for why a new file, not an extension of `test_binner.cpp` or
`test_field_pipeline.cpp`). Two source files plus its test — within the
3-file cap. (`CMakeLists.txt` also edited, to register the new test
executable — does not count against the cap, same convention WU-34c's own
entry already used.)

New test file, not an extension of an existing one — the choice this
entry's own prior-session note flagged as needing to be weighed, weighed
directly this session: `test_binner.cpp` is `core/binner.hpp`/`.cpp`'s
own file (WU-33a's job, not this one's); `test_field_pipeline.cpp` is
field-mode-*specific* correctness (interlace extract/interleave), not a
natural home for a `PipelineParams` field's own wiring question, and its
own existing tests build no independent-recomputation helper this unit's
tests could reuse. `test_pipeline_shading.cpp` itself was considered and
rejected: its own `resolveFrameIndependently()`/
`resolveParityIndependently()` helpers are `shadingGrid`-shaped, not
`backSrc`-shaped, and bolting a second, unrelated optional parameter onto
helpers written for a different field risks exactly the "quietly drift
apart" failure mode `core/pipeline.cpp`'s own file header already flags
for a different pair of hand-duplicated functions — a new file, mirroring
WU-34c's own precedent of "one new `PipelineParams` field, one new
sibling test file" exactly, was judged the cleaner choice.

**Accept:** `backSrc == nullptr` (explicit or implicit) produces
`runFrame()`/`runFrameField()` output byte-for-byte identical to its
absence entirely, for both entry points
(`test_pipeline_backsrc_default_null_preserves_runframe_output()`,
`...runframefield_output()`). Against a self-folding `buildSphereLattice()`
lattice (the same fixture `tests/test_binner.cpp` uses for its own
`backSrc` test, WU-33a) paired with a denser, 20x20 uniform source raster
(found necessary this session, not assumed — the 3x3 source
`test_binner.cpp` uses is dense enough for a fragment-level check but not
for a *composited*, post-splat/resolve difference to be visible, since
its one back-facing sample lands right at the fold's own silhouette
boundary where density-compensation weight collapses toward zero; see
`tests/test_pipeline_backsrc.cpp`'s own comment): `runFrame()`'s and
`runFrameField()`'s own output each match an independent recomputation
through `generateFragments()`/`generateFragmentsFieldRows()` (never
calling `core/pipeline.cpp`'s own private `resolveOneTile()`/
`runThreaded()`) built with the same `backSrc`, and each differs from the
same recomputation with `backSrc` forced to `nullptr` — ruling out a
wiring bug that silently drops `params.backSrc` on the floor and still
happens to pass the first check
(`test_pipeline_backsrc_runframe_selects_back_raster_on_self_fold()`,
`...runframefield_selects_back_raster_on_self_fold()`). Threaded path
(`params.threads = 4`) byte-identical to the `threads <= 1` oracle with
the same non-null `backSrc` (I6/ADR-015) —
`test_pipeline_backsrc_threaded_matches_single_threaded()`, the direct
check of `runThreaded()`'s own new parameter actually reaching every
row-band worker.

Verified non-vacuous directly: temporarily forced `runFrame()`'s own
`threads <= 1` branch to pass `nullptr` in place of `params.backSrc` at
both its `generateFragments()`/`generateFragmentsTagByFacing()` calls
(the parameter still declared, its job disabled for that one branch) and
confirmed exactly six checks failed — the self-fold `runFrame()` test's
own two checks (matches independent recomputation; differs from the
`backSrc == nullptr` recomputation) and all three of the
threaded-vs-single-threaded checks (`runThreaded()`'s own separate
`backSrc` wiring was untouched by this mutation, so the two branches now
legitimately disagreed) — with every other check, including both
`runFrameField()` checks and both "default nullptr" checks, still
passing, exactly as expected since neither is reachable through the one
mutated branch. Reverted, confirmed the restored file byte-for-byte
identical to its pre-mutation state, and reconfirmed 30/30 `ctest`
targets and 17/17 `test_pipeline_backsrc` checks green before finalising.

*Status:* `green` — built and cloud-sandbox-tested this session (Session
77): fresh copy of `origin/main` at `4e4a5ac` (WU-33a's own commit, tip
of `main` at this session's start) with these files overlaid. GCC 13.3.0
Release and Debug, Clang 18.1.3 Release and Debug, plus GCC Debug
`-fsanitize=address,undefined -fno-sanitize-recover=all` (`nm -D`/`ldd`
confirm genuine ASan instrumentation linkage) — five configurations, all
clean, zero warnings, full 30/30 `ctest` suite green in every
configuration (up from 29/29 — the new `test_pipeline_backsrc`),
`test_binner` itself unchanged at 40439 checks in every configuration,
confirming this unit's own files (`core/resolve.hpp`/`core/pipeline.cpp`)
left `core/binner.hpp`/`.cpp` untouched. All four changed/added files
(`src/core/resolve.hpp`, `src/core/pipeline.cpp`,
`tests/test_pipeline_backsrc.cpp`, `CMakeLists.txt`) written back to the
real repository via the device bridge, re-staged and diffed byte-for-byte
against the cloud sandbox's own copy — identical; the real repository's
own just-written copy was then independently re-staged and rebuilt from
scratch in the cloud sandbox a second time (fresh configure, fresh
`ninja`, GCC Release), confirming the write-back itself introduced no
corruption: 30/30 green again. Not yet built, run, tagged or pushed at
Steve's own real terminal.

### WU-33c1 — unpackSourceRaster()/OwnedSourceRaster: the v210-unpack-to-SourceRaster prefix runFrameBytes()/runFrameBytesDeinterlaced() already run, factored into a reusable, caller-owned form `green`
**Split from WU-33c this session (Session 78) — see `DECISIONS.md` ADR-094 for
the full re-derived scope.** WU-33c's own note above (Session 76) named WU-28d
as "the same treatment" this unit should follow; re-checked directly this
session, not assumed: WU-28d's own status is *superseded by WU-35a before
ever being built, run, committed or tagged* (this file's own WU-28d entry) —
it never actually established a device-bridge/real-terminal build pattern to
follow. The real precedent is WU-35a2/WU-35a3/WU-35a4 (folded into one
commit/tag, `wu-35a4-green`): sandbox-verify whatever is genuinely
hardware-independent, hand the DeckLink-linked remainder to Steve's own
real-terminal build with exact commands. Re-reading
`io/decklink_capture_consumer.hpp`/`.cpp` and `io/decklink_input.hpp`/`.cpp`
directly (not assumed from WU-36's own sweep) confirmed WU-33c's own
footprint finding still holds: `CaptureConsumer` holds exactly one `Lattice`
by value, one `PipelineParams` by value, one owned `Deinterlacer` — no notion
of a second feed anywhere in either file.

A second, independent capture-source-and-consumer pair genuinely needs new
production code, not just a second instance of the existing classes:
`CaptureConsumer` runs the *whole* front-side pipeline (unpack, deinterlace,
warp via `runFrame()`, pack) and produces output v210 bytes — the wrong shape
for a "back" source, which only ever needs to become a
`PipelineParams::backSrc` (WU-33a/WU-33b, ADR-092/093), i.e. stop at an owned
`SourceRaster`. Reading `core/pipeline.cpp`'s own
`runFrameBytes()`/`runFrameBytesDeinterlaced()` directly found that the exact
prefix a second consumer would need — v210 unpack, chroma upsample, the
WU-40/WU-41 (ADR-085) RGB boundary conversion that builds `SourceRaster` —
already exists in both functions, but only as unreachable locals (the same
kind of gap CORRECTIONS.md C-027 already found blocking
`runFrameBytesDeinterlaced()` from reusing `runFrameBytes()` directly for a
different reason), and is exactly the "candidate for consolidation if a third
caller ever needs the same sequence" `runFrameFile()`'s own header comment
(`core/resolve.hpp`) already named speculatively without committing to it.

This unit is that third caller's own dependency, split out on its own because
it is the one genuinely hardware-independent, sandbox-buildable-and-testable
piece of WU-33c's real footprint — everything else (the second
`CaptureSource`/new back-consumer class, and wiring it into the front
`CaptureConsumer`) needs `BLACKMAGIC_SDK_DIR`/Cocoa and is scoped separately
below (WU-33c2/WU-33c3), device-bridge hand-off only, following WU-33c's own
original four-plus-file-cap finding, still correct.

**Design, decided and built this session:** `OwnedSourceRaster`
(`core/resolve.hpp`) — owns a `video::RasterRGB`, exposes `view()` returning a
fresh `SourceRaster` recomputed from that storage's own current `.data()`
addresses each call, rather than a cached raw-pointer member — the same
technique `tests/test_pipeline_backsrc.cpp`'s own `UniformSource::view()`
already uses, for the same reason: a cached member would go stale across a
copy (`RasterRGB`'s `std::vector` members reallocate at a new address on
copy), and `view()` recomputing from `rgb` needs no hand-written copy or move
constructor to stay correct under either — checked directly this session
(test 2, below). `unpackSourceRaster()` (`core/pipeline.cpp`) reproduces
`runFrameBytes()`'s own first three steps (v210 unpack, chroma upsample, RGB
boundary conversion) exactly, stopping where that function goes on to call
`runFrame()`.

Deliberately built as a new, additive function rather than a refactor of
`runFrameBytes()`/`runFrameBytesDeinterlaced()` to share it: both are
already-green, already-tested call sites, and `SESSION-PROTOCOL.md`'s
anti-drift rule 2 is read the same way ADR-092's own WU-33a session already
read it for a structurally identical choice — an addition next to tested
code, not a change to it, carries this unit's own risk alone. This unit's own
test (below) verifies the duplication stays honest instead.

**Files:** `src/core/resolve.hpp` (`OwnedSourceRaster` + `unpackSourceRaster()`
declaration), `src/core/pipeline.cpp` (`unpackSourceRaster()` definition).
`tests/test_unpack_source_raster.cpp` (new, doesn't count against the cap).
`CMakeLists.txt` (registers the new test executable, doesn't count against
the cap).

**Accept — verified this session:**
1. `unpackSourceRaster()`'s own output, fed by hand into `runFrame()` and the
   same RGB->YCbCr/chroma-downsample/v210-pack tail `runFrameBytes()` already
   runs, reproduces `runFrameBytes()`'s own output bit-for-bit for a genuine,
   off-centre-warp zone plate (`tools/testpat.hpp`, the same content
   `tests/test_pipeline_bytes.cpp`'s own check 1 already uses for the same
   reason — the identity map's degenerate case would not exercise real
   fragment generation) — never calling `runFrameBytes()` itself, only its
   own already-independently-tested building blocks.
2. `OwnedSourceRaster::view()` stays correct after a copy: copied, the
   original heap-allocated and explicitly destroyed, the copy's own `view()`
   still reads back the exact values it was built with.

7 checks, both passing. Verified this session to actually catch a
regression, not pass vacuously: temporarily mutated `unpackSourceRaster()` to
feed the luma plane into all three `ycbcrToRgbImage()` input channels (a
wrong-but-plausible copy-paste mistake) and confirmed exactly one check
failed (check 1's own byte-equality) — check 2 (copy-safety) correctly
unaffected, since it does not depend on the RGB conversion being correct,
only reproducible. Reverted, confirmed the restored file byte-for-byte
identical to its pre-mutation state, rebuilt to 7/7 green again.

Full portable matrix green in the cloud sandbox: GCC 13.3.0 and Clang 18.1.3,
Release and Debug, plus GCC 13 Debug
`-fsanitize=address,undefined -fno-sanitize-recover=all` — five
configurations (matching ADR-093's own five, not ADR-092's own four — this
unit's own risk surface is a genuinely new function, not one selection
expression in an existing one), all clean, zero warnings, full 31/31 `ctest`
suite green in every configuration (up from 30/30 — the new
`test_unpack_source_raster`), `nm -D`/`ldd` confirming genuine ASan/UBSan
instrumentation linkage, `test_binner` unchanged at 40439 checks in every
configuration (confirming `core/binner.hpp`/`.cpp` untouched).
`core/resolve.hpp`/`core/pipeline.cpp` touch no DeckLink-linked file, so this
is a real, standard `scatter-core` build/test run.

All four changed/added files written back to the real repository via the
device bridge, then re-staged and diffed byte-for-byte against the cloud
sandbox's own working copy — identical. Additionally (this session's own
device bridge reaches a real Linux VM on Steve's own Mac, not only a cloud
sandbox, but that VM has no `cmake`/`ninja`/`clang` — only a third,
independent GCC, 11.4.0): the real repository's own just-written files were
compiled and run directly with that VM's own `g++ 11.4.0`, by hand (no
CMake), against the exact same compile flags CMake itself uses — 7/7 green,
zero warnings, plus `test_binner` (40439, unchanged), `test_pipeline_backsrc`
(17, unchanged) and `test_smoke` (2076, unchanged) rebuilt and passing
against the same just-written files, from the real repository directly, not
the sandbox copy. Not equivalent to a fifth configuration in `close.sh`'s own
sense (no `ctest`, no `-Werror` cross-checked against CMake's own flag set
beyond copying them by hand) but a genuine, independent confirmation that the
just-written files build and pass on the real machine, in a different GCC
version than either sandbox configuration used — Steve's own real-terminal
`close.sh` run is still the final word regardless. Scratch object files this
check produced were written outside the mounted repository folder and
removed before session end; none were ever committed.

### WU-33c2 — AMENDED (Session 79, DECISIONS.md ADR-095): a pluggable back source, not a single hard-wired second live DeckLink device
**Steve's own amendment, this session, logged as a plan amendment (the same
verdict `docs/wu-audit-2026-08.md`'s own table already uses) before any new
`Files:`/`Accept:` was written for this unit's own descendants.** The note
below (Session 78) sketched a single new DeckLink-linked back-consumer class
against a second, independently-obtained `IDeckLinkInput`, implicitly a
second physical device — motivated by Steve's own UltraStudio 4K Mini
exposing only one capture path. Steve named that as a property of *that*
device, not of the Blackmagic SDK in general: some DeckLink cards expose
multiple independent capture paths on one card, and a system can also have
multiple separate capture-capable devices attached. The amended requirement:
support at least two concrete back-source producers behind one
config-selectable choice — (1) a Blackmagic capture sub-device, enumerated
generically (by index or stable identifier, not "device 1 vs device 2") so
it handles a multi-sub-device card and multiple physical devices identically,
staying Blackmagic-only; and (2) a static v210 file. See `DECISIONS.md`
ADR-095 for the full account, including why pluggability is a plain
`std::function<OwnedSourceRaster()>` callable rather than a new virtual
interface, and why every producer returns a real copy rather than a cheap
view.

Split into two sibling units below, mirroring this project's own repeated
precedent for exactly this seam (WU-28's a/b/c/d split, WU-35a's a1/a2/a3/a4
split, WU-33c's own c1/c2/c3/c4 split last session): WU-33c2a, a static v210
file — genuinely hardware-independent, so it is sandbox-buildable-and-
testable now — and WU-33c2b, a Blackmagic-sub-device producer — DeckLink-
linked, device-bridge hand-off only, same as WU-33c2's own original note.
This entry (the original WU-33c2 note, Session 78) is retired in favour of
the two entries immediately below; `WU-33c3`/`WU-33c4` below have their own
dependency lines updated to name both.

### WU-33c2a — FileBackSource: a static v210 file as one pluggable back-source producer, built on `unpackSourceRaster()` (WU-33c1) `green`
**Split from the amended WU-33c2 this session (Session 79) — see
`DECISIONS.md` ADR-095 for the full re-derived scope.** Genuinely
hardware-independent — no `BLACKMAGIC_SDK_DIR`/Cocoa dependency at all — so,
unlike WU-33c2b below, this half of the amended WU-33c2 is sandbox-
buildable-and-testable this session, the same reasoning that already split
WU-33c1 out of the original WU-33c note (ADR-094).

Design, decided and built this session: `FileBackSource`
(`io/file_back_source.hpp`/`.cpp`) reads one static v210 frame (raw packed
bytes, tight stride, no file header) once, at construction via
`FileBackSource::create()` (a factory returning
`std::optional<FileBackSource>`, since `OwnedSourceRaster` — WU-33c1 — has no
default constructor to leave the object in a "not yet loaded" state on
failure), and its own `currentSourceRaster()` returns a copy of that same
frame's own `OwnedSourceRaster` on every call — matching the
`std::function<OwnedSourceRaster()>` shape DECISIONS.md ADR-095 decided
WU-33c3's future front-consumer wiring should take. The raw-byte file read
is a short, independent duplication of `io/file_source.cpp`'s own
`readV210File()` file-read half (WU-05) — checked directly this session,
not assumed: `readV210File()` unpacks straight into caller-supplied 4:2:2
planes via `v210::unpackImage`, with no way to hand the raw packed bytes
back to a caller, and `unpackSourceRaster()`'s own precondition (WU-33c1)
is exactly those raw bytes — so `readV210File()` cannot feed
`unpackSourceRaster()` directly, confirming the exact question the incoming
prompt asked to have settled. `SESSION-PROTOCOL.md`'s anti-drift rule 2 is
read the same way ADR-092's WU-33a session and ADR-094's WU-33c1 session
already read it: an addition next to tested code, not a change to it,
carries this unit's own risk alone.

**Explicitly out of scope, per Steve's own choice this session:** a looped
or advancing multi-frame sequence. This unit is the static-single-frame
case only; a future sibling unit (not numbered this session) covers genuine
looped video-file playback.

**Files:** `src/io/file_back_source.hpp` (new — `FileBackSource` class
declaration), `src/io/file_back_source.cpp` (new — implementation).
`CMakeLists.txt` (registers the new scatter-core source file and the new
test executable, doesn't count against the cap). `tests/test_file_back_source.cpp`
(new, doesn't count against the cap).

**Accept — verified this session:**
1. `currentSourceRaster()`'s own output, read back from a real file on disk,
   reproduces `unpackSourceRaster()`'s own output (WU-33c1) bit-for-bit for
   the exact same packed bytes held only in memory.
2. `currentSourceRaster()` called twice on the same `FileBackSource` returns
...
4. `create()` returns `std::nullopt` for a file shorter than
   `v210::rowBytesMin(width) * height` bytes (truncated).
5. `create()` returns `std::nullopt` for an unsupported (odd) width.
6. `create()` returns `std::nullopt` for a non-positive height.

8 checks, all passing. Verified this session to actually catch a
regression, not pass vacuously: temporarily mutated `create()` to unpack a
zeroed buffer instead of the real bytes just read from disk, and confirmed
exactly one check failed (check 1's own byte-equality comparison) — check 2
(which compares the mutated provider against itself) and all four
failure-path checks correctly unaffected. Reverted, confirmed the restored
file byte-for-byte identical to its pre-mutation state, rebuilt to 8/8 green
again.

Full portable matrix green in the cloud sandbox: GCC 13.3.0 and Clang 18.1.3,
Release and Debug, plus GCC 13 Debug
`-fsanitize=address,undefined -fno-sanitize-recover=all` — five
configurations, all clean, zero warnings, full 32/32 `ctest` suite green in
every configuration (up from 31/31 — the new `test_file_back_source`),
`nm -D`/`ldd` confirming genuine ASan/UBSan instrumentation linkage,
`test_binner` unchanged at 40439 checks in every configuration. This
session's own cloud sandbox additionally confirmed a genuine `git clone` of
`origin` now succeeds directly (resolving the "no working git-clone
credentials" limitation Sessions 77/78 both flagged) — used in place of the
tar/stage workaround this time.

All new/changed files written back to the real repository via the device
bridge, then re-staged and diffed byte-for-byte against the cloud sandbox's
...
### WU-33c4 — FileBackSource wired into a real, observable front/back demo `todo` — written this session, not built

**Re-derived against the real current code this session (Session 82) — this note's own prior sketch (Session 78, written before WU-33c2a/WU-33c2b/WU-33c3 had even split or decided their own real shapes) assumed extending `tests/test_decklink_live_sphere.cpp` in place and a command-line flag choosing between a static file and a Blackmagic sub-device; neither survived re-derivation. See `DECISIONS.md` ADR-098 for the full account, not repeated in full here.**

**Amended later the same session (`DECISIONS.md` ADR-099), after Steve's first real-terminal run raised three things.** Two are parked, named here so they are not lost: (1) `firstFormatDetectionCapableInput()`'s own "take the first match" device-selection convention — shared by every DeckLink test in this repository, this one included — turns out to be a real, live problem on Steve's own machine now that it enumerates more than one capture-capable device and the first match isn't always the fed one; this also answers WU-33c5's own open hardware question below (yes, more than one such device exists), without fixing the selection bug itself. (2) A frame-rate drop Steve attributed to "the front/back processing," not narrowed down this session — one named, not-yet-confirmed candidate for whoever picks it up: `FileBackSource::currentSourceRaster()` returns a full owned-frame copy by value on every `processOne()` call (WU-33c2a's own already-documented "deliberate, named cost," accepted before any session had a real frame rate to weigh it against). The third, acted on: Steve asked for `test_decklink_live_sphere.cpp`'s own interactive keyboard controls ported into this file — done, replacing the original automatic sweep as the primary path (see below).

**Scope narrowed to `FileBackSource` (WU-33c2a) only — `DeckLinkBackSource` (WU-33c2b) split out to a new sibling unit, WU-33c5, below.** `DeckLinkBackSource` needs a second, genuinely distinct capture-and-format-detection-capable DeckLink input, which Sessions 79 through 82 have all been unable to confirm Steve's own hardware actually enumerates — no session has had real hardware access to run `enumerateDeckLinkDevices()` and look. Rather than guess, this unit proves the visual "different content front vs back" claim the way `FileBackSource` alone already can (Steve's existing single UltraStudio, no second device), and the two-device half is named and deferred, not silently dropped — the same "named, split out, not silently expanded" move this project's whole WU-33c lineage has already used at every prior fork (WU-33c1 off WU-33c, ADR-094; WU-33c2a/WU-33c2b off WU-33c2, ADR-095).

**New file, not an extension of `tests/test_decklink_live_sphere.cpp`.** That file is 909 lines as of this session — confirmed by direct `wc -l`, not assumed — and `SESSION-PROTOCOL.md`'s own per-test-file fixture isolation convention (rule 2) is what every existing DeckLink test in this repository already follows for a structurally similar reason. `tests/test_decklink_live_backsrc_demo.cpp` duplicates `test_decklink_live_sphere.cpp`'s own `makeSphereLattice()`/`rotateLattice()` locally (confirmed this session: the existing sphere, full pole-to-pole/360-wrap, already exposes real back-facing samples via `core/binner.hpp`'s own `surfaceNormal(rawJ).z >= 0.0` selection — no new shape needed). **ADR-099 (same session): this file now also duplicates `test_decklink_live_sphere.cpp`'s own `Key`/`readKey()` and flag-off (non-`CoverageWindow`) interactive loop** — cursor keys for yaw/pitch, X/x/Y/y for position, Z/z for radius, T/t for `manualTransp`, Q to quit — at Steve's own explicit request, once it was clear an unattended fixed-speed sweep was a worse way to actually inspect the front/back effect than driving it by hand. `CoverageWindow`/`--show-coverage` (WU-22c) was deliberately not ported — a materially separate, unasked-for feature. The original automatic 30-second yaw sweep is kept, demoted to the non-interactive-terminal fallback rather than deleted.

**No command-line flag; `kBufferMode`/`frontTag`/`backTag` configuration added later the same session, for a different reason than originally expected.** Confirmed directly against `core/pipeline.cpp` this session, not assumed: `runFrame()`/`runFrameField()` forward `params.backSrc` to whichever of `core/binner.hpp`'s six `generateFragments*()` entry points they call *unconditionally* — the plain, single-scalar-tag, `kBufferMode == Off` path included, so `backSrc` substitution itself never needed any k-buffer configuration. WORK-UNITS.md's own prior WU-33c4 sketch assumed a command-line selection surface would be needed here — re-derived away, since with only one producer in scope there is nothing left to select between; no such flag exists. `params.kBufferMode = Blend` was added anyway, by ADR-099, purely so the newly-ported T/t controls (above) have a visible effect at all — `frontTag`/`backTag` are still left at their default (both 0), since that gate gets used only by the facing-tag fragment-generation choice, not by the Blend resolve itself.

**Back-side content: a real WU-03 zone plate (`tools/testpat.hpp`), generated and written to a temp `.v210` file at startup, then read back through `FileBackSource::create()`** — mirrors `tests/test_decklink_output.cpp`'s own `writeWarpedTestFrame()`, which already uses exactly this generate-then-write-then-read-back shape. Chosen over a solid colour (e.g. `video::packBlackFrame`, WU-33c3's own test) specifically because this unit's whole point is a by-eye "genuinely different content" check, and a zone plate cannot coincidentally match whatever a live signal happens to show.

**Files:** `tests/test_decklink_live_backsrc_demo.cpp` (new — the unit's own test/demo executable; does not count against the 3-source-file cap, the same convention every prior WU-33c-lineage unit's `Files:` line already applies to its own test file). `CMakeLists.txt` (modified — new `add_executable`/`target_link_libraries`/`target_include_directories`/`add_test` block, mirroring `test_decklink_live_sphere`'s own dual-link shape minus `scatter-diag`, plus `tools/` on the include path for `testpat.hpp`, the same reason `test_decklink_output` already adds it). **Zero `src/` files touched** — confirmed this session: every piece this unit wires together (`CaptureConsumer::BackSourceCallback`, `FileBackSource`, `LiveFramePlayback`, `testpat::makeZonePlate()/writeV210()`) already exists exactly as WU-33c3/WU-33c2a/WU-21c/WU-03 left it; this unit is pure composition. Comfortably inside the 3-source-file cap (0 of 3 used) — no split into WU-33c4a/WU-33c4b needed.

**Accept — written this session, NONE of it run or confirmed (DeckLink-linked, device-bridge hand-off only, same limitation as WU-33c2b/WU-33c3):** by-eye, at Steve's own real terminal (the same "no automated test can observe an SDI monitor" limitation WU-28d's own original `Accept:` line already named) — the near (front-facing) hemisphere of the sphere shows the live front feed, the far (back-facing) hemisphere shows the static zone plate, and the two visibly swap as the sphere is rotated — now via the ported interactive controls (cursor keys/X/x/Y/y/Z/z/T/t/Q) at a real terminal, or via the original automatic 30-second yaw sweep when stdin is not a real terminal. Automated `CHECK`s cover the mechanics only (device enumeration, clean start/stop, the same `framesProcessed + framesFailed + framesStreamStart == framesPopped` accounting invariant and the `backSourceQueried` invariant `test_capture_consumer_queries_wired_back_source()` already established, WU-33c3) — these hold trivially at zero on a no-signal run, the same "nothing plugged in is a real, honestly reportable state" convention every real-hardware test in this repository already uses, and are not themselves proof of the by-eye claim. **Do not claim the by-eye front/back distinction works until Steve has built, run, and confirmed it at his own real terminal, watching the Monitor 3G output** — and note the device-selection question above may need resolving first if the front input picked is not the one actually fed.

### WU-33c5 — `DeckLinkBackSource` wired into a real, two-device front/back demo `todo`

**Split out of WU-33c4 this session (Session 82, DECISIONS.md ADR-098) — deliberately not scoped past this note, the same "named, split out, not silently expanded" move used at every prior WU-33c fork.** Depends on WU-33c2b (already written, `DECISIONS.md` ADR-096) and on a real answer to the question Sessions 79 through 82 have all carried forward unresolved: does Steve's own hardware setup currently enumerate more than one capture-and-format-detection-capable entry from `enumerateDeckLinkDevices()`? No session so far has had real hardware access to check. Whoever picks this up should confirm that directly against Steve's own real terminal before writing a `Files:`/`Accept:` line — if only one such device exists, this unit cannot be built as scoped and should say so plainly rather than substitute `FileBackSource` again (WU-33c4 already covers that case). Likely close in shape to `tests/test_decklink_live_backsrc_demo.cpp` (WU-33c4) — a second `CaptureFrameRing`/`CaptureSource` against the second selected input, feeding a `DeckLinkBackSource` (WU-33c2b) instead of a `FileBackSource`, wrapped in a `BackSourceCallback` the same way — but re-derive against the real current code by then, per this whole lineage's own standing instruction, rather than trust this paragraph's own sketch.

**Accept:** provisional, by-eye, two real live feeds required (front UltraStudio input plus a second real capture-capable device or sub-device) — device-bridge hand-off only, not scoped further here.

### WU-33c6 — copy-on-write storage for `OwnedSourceRaster`, removing `FileBackSource`'s own per-frame full-buffer copy `green`

**Built and tested this session (Session 83), on top of this file's own prior `todo` scoping (Session 82, `DECISIONS.md` ADR-100) — re-derived against the real current code before writing anything, per this whole project's own standing instruction, not assumed still accurate.** One minor discrepancy found by that re-derivation: ADR-100/this unit's own prior note said `unpackSourceRaster()`'s own touched lines in `core/pipeline.cpp` numbered five; the real count, confirmed by direct read and by the diff this session's own edit actually produced, is two physical lines (`out.rgb.R.data(), out.rgb.G.data(), out.rgb.B.data(),` and `out.rgb.planeR().strideSamples);`), four `.` → `->` conversions across those two lines. Not logged as a `CORRECTIONS.md` entry — a descriptive-prose miscount in the prior session's own scoping note, not a wrong `Files:`/`Accept:` line, not a code or test defect, the same threshold `docs/wu-audit-2026-08.md`'s own WU-26 sweep already drew for a comparably minor cosmetic finding (a stale comment, logged in that doc's own table, not in `CORRECTIONS.md`). The scope, files and design were otherwise exactly as ADR-100 described.

**`src/core/resolve.hpp`: `OwnedSourceRaster::rgb` changed from `video::RasterRGB` (owned by value) to `std::shared_ptr<video::RasterRGB>`** (first use of `std::shared_ptr` anywhere in `src/`, confirmed by grep both before and after this change), constructed via `std::make_shared<video::RasterRGB>(width, height)`. Added `#include <memory>`. `view()`'s body changed from `rgb.width`/`rgb.R.data()` etc. to `rgb->width`/`rgb->R.data()` etc. — otherwise identical, still recomputing a fresh `SourceRaster` from `rgb`'s own current addresses on every call, exactly as ADR-094 established. The struct's own header comment was extended, not replaced, to record the ADR-100/WU-33c6 design and why it is safe (nothing mutates an `OwnedSourceRaster` after construction).

**`src/core/pipeline.cpp`: `unpackSourceRaster()`'s own two `out.rgb.` lines became `out.rgb->`** — mechanical only, no logic change, exactly as scoped.

**`tests/test_unpack_source_raster.cpp`: extended, not replaced.** WU-33c1's own two existing checks (bit-identical output vs. `runFrameBytes()`; a copy's own `view()` survives the original's destruction) kept verbatim in intent — check 2's own inline comments updated in place to say plainly that the original's destruction now frees nothing (the underlying storage is kept alive by the copy's own shared reference), rather than leave a now-false "deep copy"/"genuinely freed" comment standing uncorrected. Two new checks added, exactly as ADR-100 scoped: (3) `test_owned_source_raster_copy_shares_storage_pointer_equal()` — a copy's own `view().r`/`.g`/`.b` compare pointer-*equal* to the original's; (4) `test_owned_source_raster_independent_instances_do_not_share_storage()` — two separately-constructed `OwnedSourceRaster`s (two independent `unpackSourceRaster()` calls, not copies of one another) compare pointer-*unequal*. The file's own header comment ("Two checks") extended to "Four checks", describing 3/4 alongside the original two.

**Files touched: exactly the three named in this unit's own prior `Files:` line** (`src/core/resolve.hpp`, `src/core/pipeline.cpp`, `tests/test_unpack_source_raster.cpp`) — **zero files touched in `io/`**, confirmed again by grep this session: `FileBackSource`, `DeckLinkBackSource`, and `CaptureConsumer` all still treat `OwnedSourceRaster` purely as an opaque returned/copied value; none needed a call-site change.

**Built and run for real this session, not merely scoped — full account in `DECISIONS.md` ADR-101 (new entry, does not revise ADR-100):**

- Baseline confirmed first: a fresh `git clone` of `origin` at this unit's own starting commit (`aac353c`, "WU-33c6 scoped (not built)") built clean and `test_unpack_source_raster` ran 7/7 green in the cloud sandbox, GCC 13.3.0 Release — confirming the starting point matched this file's own account before any edit was made.
- After this unit's three edits: **GCC 13.3.0 Release — full 32/32 `ctest` suite green** (`test_unpack_source_raster` itself now 13 checks, up from 7 — the six new `CHECK`s from tests 3/4). **Clang 18.1.3 Release — full 32/32 `ctest` suite green.** **GCC 13.3.0 Debug, `-fsanitize=address,undefined -fno-sanitize-recover=all` — full 32/32 `ctest` suite green**, `nm -D`/`ldd` confirming genuine ASan/UBSan instrumentation linkage. Zero compiler warnings in any configuration.
- Verified this session to actually catch a regression, not pass vacuously, the same discipline WU-33c1/WU-33c2a's own sessions already used: temporarily reverted `OwnedSourceRaster::rgb` to an owned-by-value `video::RasterRGB` (simulating the pre-this-unit type) and confirmed exactly check 3's own three assertions failed, nothing else — reverted, confirmed the restored files byte-for-byte identical to their pre-mutation state, rebuilt to 13/13 green again. Separately, mutated the constructor to cache one `static` `shared_ptr` shared across every instance (simulating a broken single-shared-buffer implementation) and confirmed exactly check 4's own three assertions failed, check 3 unaffected — reverted the same way, confirmed byte-identical, rebuilt to 13/13 green again.
- All three changed files written back to the real repository via the device bridge, then re-staged and diffed byte-for-byte against the cloud sandbox's own working copy — identical. Additionally, independently of the cloud sandbox: the real repository's own just-written files were compiled directly on the device bridge's own Linux VM with its bare `g++ 11.4.0` (no CMake there, same as every prior session's own finding), by hand, against this project's own full `-Wall -Wextra -Wpedantic -Wconversion -Wsign-conversion -Werror`/`-std=c++20` flag set — `test_unpack_source_raster` 13/13 green, zero warnings, plus `test_file_back_source` (8/8, unchanged) and `test_pipeline_backsrc` (17/17, unchanged) rebuilt and passing against the same just-written files. Scratch object/binary files this check produced were written outside the mounted repository folder (this device bridge session's own `$HOME`, not `mnt/`) and removed before session end; none were ever committed.

**Explicitly not claimed, same as ADR-100/ADR-044/WU-19b: any frame-rate/timing number, from either sandbox.** What this unit proves, now for real rather than merely argued: the copy this session's own build confirms was real is gone, replaced by an atomic refcount increment, with the full 32-test suite honestly green around it in three independent configurations plus a fourth independent compiler on the real hardware. **Whether this was actually the cause of Steve's own reported frame-rate drop, and whether `framesPushed`/`framesArrived`'s own ratio improves, is still his own job to confirm** at his own real terminal, per ADR-100's own account — unchanged by this unit actually being built.

**Not committed, tagged, or pushed by this session** — per this project's own standing instruction, that is Steve's own job; exact commands in `HANDOFF.md`.
