...
**New file, not an extension of `tests/test_decklink_live_sphere.cpp`.** That file is 909 lines as of this session — confirmed by direct `wc -l`, not assumed — and `SESSION-PROTOCOL.md`'s own per-test-file fixture isolation convention (rule 2) is what every existing DeckLink test in this repository already follows for a structurally similar reason. `tests/test_decklink_live_backsrc_demo.cpp` duplicates `test_decklink_live_sphere.cpp`'s own `makeSphereLattice()`/`rotateLattice()` locally (confirmed this session: the existing sphere, full pole-to-pole/360-wrap, already exposes real back-facing samples via `core/binner.hpp`'s own `surfaceNormal(rawJ).z >= 0.0` selection — no new shape needed) but drops the interactive keyboard control surface entirely (`test_decklink_live_sphere.cpp` already owns that, WU-21h/21i/22c/35a2/35a3) in favour of one automatic 2π yaw sweep over a fixed 30-second window — this unit's own job is the `backSrc` wiring proof, not another control surface.

**No command-line flag, no `kBufferMode`/`frontTag`/`backTag` configuration.** Confirmed directly against `core/pipeline.cpp` this session, not assumed: `runFrame()`/`runFrameField()` forward `params.backSrc` to whichever of `core/binner.hpp`'s six `generateFragments*()` entry points they call *unconditionally* — the plain, single-scalar-tag, `kBufferMode == Off` path included. `backSrc` substitution and k-buffer facing-tag/blend are two independent mechanisms; this unit needs only the former, so default `PipelineParams` (plus `destWidth`/`destHeight`) is genuinely enough. WORK-UNITS.md's own prior WU-33c4 sketch assumed a command-line selection surface would be needed here — re-derived away, since with only one producer in scope there is nothing left to select between.
