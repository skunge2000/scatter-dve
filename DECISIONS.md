...
**ADR-097 — WU-33c3: `CaptureConsumer::BackSourceCallback` — reconciling `FileBackSource`/`DeckLinkBackSource`'s differing accessor shapes, and the new constructor parameter's own type.**

This session (Session 81) opened by verifying the real repository state directly rather than trusting the incoming prompt's account of it: `HEAD` and `origin/main` both `f733374` (WU-33c2b's own commit, Session 80), confirming Steve had already committed and pushed WU-33c2b since the last session — not the incoming prompt's "not committed yet" fallback branch. Checked directly, not assumed: `wu-33c2a-green` is tagged, on `e1060ac`; `wu-33c2b-green` is **not** yet tagged, on `f733374` — a new instance of the same recurring pattern this project's own `HANDOFF.md` lineage keeps finding (Sessions 77 through 80 each found one to three units deep; this session finds one, one unit later in the chain). No stale `.git/index.lock` this session — checked directly, none found; `.git/index.lock` was not present at any point this session. `HANDOFF.md` was the only dirty file at session start (Session 80's own draft), never itself a blocker per `SESSION-PROTOCOL.md`. `docs/wu-audit-2026-08.md` line 113 re-checked directly this session, still stale, same standing instruction Sessions 72-80 already followed — **now nine sessions running** (72 through 81) — named again below, not fixed, still outside this session's own scope.

**Job: `WU-33c3`, re-derived against the real current code before writing any `Files:`/`Accept:`, per the incoming prompt's own explicit instruction not to trust `WORK-UNITS.md`'s own prior sketch (Session 78, predating the WU-33c2a/WU-33c2b split).** Read in full this session, not assumed from any prior session's own account: `src/io/decklink_capture_consumer.hpp`/`.cpp` (WU-21b, the class this unit modifies), `src/io/decklink_back_source.hpp`/`.cpp` (WU-33c2b), `src/io/file_back_source.hpp`/`.cpp` (WU-33c2a), `src/core/resolve.hpp`'s own `PipelineParams::backSrc`/`OwnedSourceRaster::view()`, and `src/core/pipeline.cpp`'s own `runFrameBytesDeinterlaced()` (confirming `params.backSrc` is forwarded straight through to `runFrame()`'s own PASS-1 call sites with no wiring of its own needed, WU-33b/ADR-093's own already-settled design). Confirmed by direct grep this session, not assumed: still exactly three `CaptureConsumer consumer(` construction call sites (`tests/test_decklink_capture_consumer.cpp`, `tests/test_decklink_live_output.cpp`, `tests/test_decklink_live_sphere.cpp`) — the same pattern `CORRECTIONS.md` C-028 already found a naive grep misses once, checked against the real pattern (the space before the variable name) directly.

**Design decision 1 — the two producers' differing accessor shapes are reconciled by widening `CaptureConsumer`'s own callable member to `std::function<std::optional<OwnedSourceRaster>()>`, not by wrapping `DeckLinkBackSource`'s own accessor in a fallback-substituting lambda at the wiring site.** ADR-096's own Design decision 4 left this open, naming both live options without picking one. Considered and rejected: wrapping the optional-returning `DeckLinkBackSource::currentSourceRaster()` in a caller-supplied lambda that substitutes a last-known-good or placeholder frame, keeping `CaptureConsumer`'s own member at the original `std::function<OwnedSourceRaster()>` shape ADR-095 fixed. Rejected for a concrete, not merely stylistic, reason found by re-deriving against the real code this session: `core/resolve.hpp`'s own `PipelineParams::backSrc` comment states, unchecked, that `backSrc must have the same width and height as src` — and `src` itself is built per-frame from the *front* capture's own reported `GetWidth()`/`GetHeight()` (`io/decklink_capture_consumer.cpp`'s own processOne(), "trusted directly, per frame, never checked against the display mode"), which is not fixed for this consumer's own whole lifetime and can legitimately change after a format-change restart. A fallback frame prepared once, at the wiring site, before any frame has necessarily arrived, cannot know what per-frame width/height it would need to match without already holding a real frame of the right geometry — precisely the chicken-and-egg problem `DeckLinkBackSource`'s own "not ready yet" state exists to name honestly in the first place. Widening the member to `std::optional` instead pushes no such fabrication problem onto anyone: a `std::nullopt` result for a given frame simply leaves that frame's own `callParams.backSrc` at its existing default, `nullptr` — a state every existing `PipelineParams::backSrc` caller already exercises today, byte-for-byte, since ADR-092/093 first added the field (every caller before this unit leaves it at its own default). This is not a new code path invented for this unit; it is the existing default, applied per-frame instead of globally, needing no new logic anywhere in `core/binner.hpp`/`core/pipeline.cpp`. `FileBackSource`'s own accessor, which never fails once constructed, is trivially wrapped the other direction, at the wiring site, to match the widened signature (`[&file]{ return std::optional<OwnedSourceRaster>(file.currentSourceRaster()); }`) — a one-line adaptation with no fabrication problem of its own, since it never needs to invent a value.

`processOne()`'s own resulting behaviour on a `std::nullopt` frame: leave `callParams.backSrc` unset (`nullptr`) for that frame only — the same "this frame doesn't get one" shape `m_coverageCallback` already uses when null, generalized from "the whole consumer never opted in" to "this one frame's own query came back empty," which needed no new precedent, only extending an existing one down to per-frame granularity. This is option (b) from the incoming prompt's own two named alternatives; option (a), the fallback-lambda wrapper, is the one rejected above, for the concrete width/height reason found this session, not merely preferred stylistically.

**Design decision 2 — `CaptureConsumer`'s new constructor parameter is a `BackSourceCallback` (a `std::function`), not a non-owning pointer or reference to a WU-33c2 producer.** `WORK-UNITS.md`'s own WU-33c3 note (Session 78, predating the WU-33c2a/WU-33c2b split) sketched "an optional, non-owning pointer to a WU-33c2 back consumer" — not trusted, per that note's own explicit instruction and confirmed stale this session: a raw pointer to a single concrete producer type cannot name both `FileBackSource` and `DeckLinkBackSource` behind one parameter without a common base class neither class has (this codebase's own established position, reasoned in ADR-095: pluggability here is a `std::function`, not a virtual interface — `IDeckLinkInputCallback` and friends are COM interfaces the Blackmagic SDK mandates, not a pattern this project chose for its own internal seams). A reference has the identical problem, plus the usual reference-member drawbacks (no default state to represent "no back source," which this unit's own zero-cost-when-absent requirement needs — the same reason `coverageCallback` itself is a `std::function`, defaulted to `nullptr`, not a reference, per ADR-058). Decided: `BackSourceCallback`, mirroring `CoverageCallback`'s own already-established shape exactly — a public type alias, a new constructor parameter defaulted to `nullptr`, stored as a `const` member, appended strictly after `coverageCallback` in the parameter list. Appending after, not before or between, is what lets every one of this repository's own three existing `CaptureConsumer` construction call sites keep compiling completely unchanged — confirmed directly this session against the real current call sites, not assumed: none of the three (`tests/test_decklink_capture_consumer.cpp`'s own 4-argument call, `tests/test_decklink_live_output.cpp`'s own 4-argument call, `tests/test_decklink_live_sphere.cpp`'s own 5-argument call passing `coverageCallback` explicitly) need a single line changed. This is the same "new optional capability, new trailing default parameter" shape `coverageCallback` was itself added with (WU-22c) — extended, not reinvented.

Ownership: the callable itself is caller-constructed and copied in by value at construction (`std::move`d into the member), the same convention `coverageCallback` already uses. Whatever the callable's own lambda captures (typically a reference to a caller-owned `FileBackSource`/`DeckLinkBackSource` instance) must outlive this `CaptureConsumer` — the same unchecked "caller owns it, must outlive this object" convention this class's own `ring` parameter already documents, extended here rather than reinvented, and named explicitly in this unit's own header comment.

**A real, named limitation, not newly introduced by this unit but newly load-bearing once a real second producer is wired in: `backSrc` must have the same width/height as the front side's own `src` raster, unchecked, per-frame.** `core/resolve.hpp`'s own `PipelineParams::backSrc` comment already states this precondition, unchecked, "caller's bug, not checked here" — this unit inherits it unchanged, does not add a runtime check, and does not relax it (`SESSION-PROTOCOL.md`'s own anti-drift rule 4: never relax an invariant to make a test pass — there is no test here that this would make pass, only a check this unit deliberately does not add). A front and back capture device or sub-device reporting genuinely different geometry per frame — plausible in general, since both are trusted directly, per frame, with no cross-check against each other or against any display mode — would be undefined behaviour through this wiring: out-of-bounds reads on `backSourceView`'s own r/g/b storage. Named here, not silently ignored: whoever wires two real, physically distinct capture devices together (WU-33c4's own job) should confirm both report the same geometry before trusting this path, or add a check then, against real hardware, not invented here without hardware to confirm it against.

**Written this session, confirmed byte-for-byte identical after the write, via the device bridge:** `src/io/decklink_capture_consumer.hpp` (modified — `BackSourceCallback` type alias, `backSourceQueried` stat, new constructor parameter/member), `src/io/decklink_capture_consumer.cpp` (modified — constructor initializer list, `processOne()`'s own new snapshot/wiring logic), `tests/test_decklink_capture_consumer.cpp` (modified — new `test_capture_consumer_queries_wired_back_source()`, wiring a real `FileBackSource` against a static black frame into a real `CaptureConsumer`, and a new header-comment paragraph). Each replacement applied via a Python read-modify-write script, every old-text match asserted unique before being applied, the same discipline this project's own WU-33c2a/WU-33c2b sessions already used. All three files re-read from the real repository after writing and diffed against this session's own intended content — identical. No `CMakeLists.txt` change: confirmed by direct re-read this session, not assumed — `test_decklink_capture_consumer` already links both `scatter-decklink` and `scatter-core`, and `src/io/file_back_source.cpp` (WU-33c2a) already builds into `scatter-core`, so the new test's own use of `FileBackSource` needs no new registration.

**Not built, not run, not confirmed against real hardware, by this or any prior session.** `io/decklink_capture_consumer.hpp`/`.cpp` is DeckLink-linked (`#include "DeckLinkAPI.h"`), the same limitation WU-33c2b already had: cannot be built in the cloud sandbox (no `BLACKMAGIC_SDK_DIR`, no Cocoa — reconfirmed directly this session: `cmake`/`ninja`/`clang++`/`g++` all present, `BLACKMAGIC_SDK_DIR` unset, no DeckLink SDK anywhere on the machine) or in the device bridge's own Linux VM (`device_bash`: bare `g++ 11.4.0`, no `cmake`/`ninja`/`clang++`, not macOS, no `DeckLinkAPI.h` found anywhere — reconfirmed directly this session via `find / -iname DeckLinkAPI.h`, empty). Unlike WU-33c2a's own session, none of this unit's own changed files are portable — every one of them either includes `DeckLinkAPI.h` directly or (the test) constructs a real `CaptureConsumer`/`CaptureSource` — so there was no scatter-core subset to rebuild as even a partial independent check this session, the same "nothing this unit produced is sandbox-buildable" finding WU-33c2b's own session already reached for the identical reason. This session's own cloud-sandbox `git clone` of `origin` was reconfirmed working directly (a fresh clone, not reused from a prior session's own checkout) — used only to double-check the toolchain/SDK-absence finding above, not to attempt a build of anything this unit changed. Steve's own real-terminal `close.sh` run is the first time any of this session's own C++ will actually compile.

**`docs/wu-audit-2026-08.md` line 113 re-checked directly this session, not fixed, same standing instruction Sessions 72-80 already followed** — still reads "The real-content gap (single tag per call) was already found and logged as C-020/ADR-062 before this sweep, and closed by WU-28c," still incorrect for the same reason already found (the real-content gap wasn't actually closed until WU-35a4/WU-47). Left unfixed, outside this session's own scope — **now nine sessions running** (72 through 81).

**Untouched, deliberately.** `INVARIANTS.md` (not touched, not read for a change — grepped this session for `backSrc`/`CaptureConsumer` to confirm directly, not assumed — no match, no invariant this session's own work bears on). `ADR-059/062/065/069/070/072/073/074/077/082/083/084/085/086/087/088/089/090/091/092/093/094/095/096` (not reopened; this session's own work is a new ADR, 097, not a revision of any of these). `CORRECTIONS.md` (no new entry — nothing wrong was found in the repository this session). `io/decklink_back_source.hpp`/`.cpp` (WU-33c2b), `io/file_back_source.hpp`/`.cpp` (WU-33c2a): read in full this session as this unit's own two producer shapes to reconcile — not edited. `core/resolve.hpp`, `core/pipeline.cpp`: read in part (`PipelineParams::backSrc`'s own comment, `OwnedSourceRaster`/`OwnedSourceRaster::view()`, and `runFrameBytesDeinterlaced()`'s own body, confirming `params.backSrc` needs no wiring of its own) — not edited; this unit's whole footprint is two modified `io/` files plus one modified test. `tests/test_decklink_live_output.cpp`, `tests/test_decklink_live_sphere.cpp`: read (the exact `CaptureConsumer consumer(` construction call in each) to confirm neither needs a change — not edited. `docs/wu-audit-2026-08.md` (flagged above, not fixed).


**ADR-098 — WU-33c4: FileBackSource-only front/back demo, `DeckLinkBackSource`'s own two-device demo split out to WU-33c5, and confirming `backSrc` needs no k-buffer configuration.**

This session (Session 82) opened by verifying the real repository state directly rather than trusting the incoming prompt's account of it: `HEAD` (`ef2c1d9`, WU-33c3's own commit) one commit ahead of `origin/main` (`f733374`, WU-33c2b's own commit) — WU-33c3 was already committed locally, not the incoming prompt's "not committed yet" fallback branch, and not yet pushed. Checked directly, not assumed: neither `wu-33c2b-green` (on `f733374`) nor `wu-33c3-green` (on `ef2c1d9`) is tagged — the same recurring committed-but-untagged pattern Sessions 77 through 81 already found on other units, one unit further along the chain this time. A genuine new finding this session made, not carried in from any prior account: `build/Testing/Temporary/LastTest.log` shows a complete, unmodified-by-this-session `ctest` run at Steve's own real terminal, today, 41/41 targets passing (`grep -c "Test Passed"` confirms), including `test_decklink_capture_consumer`'s own new backsrc-wiring checks (WU-33c3) — so WU-33c3 is not merely committed, it is already built and tested green at a real terminal, just not yet tagged or pushed (the tree was dirty from `HANDOFF.md`'s own draft at the time, which `tools/close.sh`'s own clean-tree precondition would have refused outright — `HANDOFF.md` is never part of a unit's own commit, per `SESSION-PROTOCOL.md`, so this is an ordinary sequencing gap, not a defect). No stale `.git/index.lock` this session — checked directly, none found at any point. `docs/wu-audit-2026-08.md` line 113 re-checked directly this session, still stale, same standing instruction Sessions 72-81 already followed — **now ten sessions running** (72 through 82), named again below, not fixed.

**Job: `WU-33c4`, re-derived against the real current code before writing any `Files:`/`Accept:`, per the incoming prompt's own explicit instruction not to trust `WORK-UNITS.md`'s own prior sketch (Session 78, written before WU-33c2a/WU-33c2b/WU-33c3 had even split or decided their own real shapes).** Read in full this session, not assumed: `tests/test_decklink_live_sphere.cpp` (909 lines, confirmed by direct `wc -l` — the closest existing real-hardware demo precedent, its own `makeSphereLattice()`/rotation math/`CoverageWindow` wiring), `tests/test_decklink_capture_consumer.cpp`'s own `test_capture_consumer_queries_wired_back_source()` (WU-33c3's own exact `BackSourceCallback` wiring pattern), `src/io/decklink_capture_consumer.hpp`/`.cpp` (confirming the real current constructor signature: `CaptureConsumer(CaptureFrameRing&, Lattice, PipelineParams, video::DeinterlaceCoefficients, CoverageCallback = nullptr, BackSourceCallback = nullptr)`), `src/io/decklink_back_source.hpp`/`.cpp`, `src/io/file_back_source.hpp`/`.cpp`, and `src/core/binner.hpp`'s own `surfaceNormal(rawJ).z >= 0.0` back-facing selection.

**Design decision 1 — which back-source producer: `FileBackSource` only, `DeckLinkBackSource` split out to a new sibling unit, WU-33c5, not scoped further here.** `DeckLinkBackSource` (WU-33c2b) needs a second, genuinely distinct capture-and-format-detection-capable DeckLink input. Sessions 79, 80, 81 and this one have all been unable to confirm Steve's own hardware actually enumerates one — no session run so far has had real hardware access to call `enumerateDeckLinkDevices()` and look at its own output. Guessing either way (assuming a second device exists and scoping WU-33c4 around it, or silently substituting `FileBackSource` into what was meant to be `DeckLinkBackSource`'s own demo) was rejected in favour of the same move this project's whole WU-33c lineage has already used at every prior fork (WU-33c1 off WU-33c, ADR-094; WU-33c2a/WU-33c2b off WU-33c2, ADR-095): name the hardware-gated half, split it into its own unit (WU-33c5), and build the half that is fully specifiable right now. `FileBackSource` alone is sufficient to prove this unit's own real claim — front and back showing genuinely different content — since it needs no second device at all, only Steve's existing single UltraStudio.

**Design decision 2 — the lattice: `test_decklink_live_sphere.cpp`'s own sphere, duplicated locally, not a new shape.** WU-33c3's own test deliberately used an identity lattice and named, in its own header comment, that an identity lattice never produces a back-facing sample. Confirmed directly this session, not assumed: `test_decklink_live_sphere.cpp`'s own sphere is a full pole-to-pole (`kAngleSpanV == M_PI`) full-360-wrap (`kAngleSpanH == 2*M_PI`) closed shape — at any single orientation, roughly half its own control vertices already satisfy `core/binner.hpp`'s own `surfaceNormal(rawJ).z >= 0.0` selection and roughly half do not, so this shape already exposes real back-facing texels without modification; no new, more deliberately reversed shape was worth inventing. `makeSphereLattice()`/`rotateLattice()` (renamed `rotateLatticeYaw()`, yaw-only, no pitch/position/radius controls — this unit has no operator surface to drive them) are duplicated locally into the new file, per `SESSION-PROTOCOL.md` rule 2 (one unit, one test, no shared fixture across test translation units — the same convention every existing DeckLink test file in this repository already follows), not extracted into a shared header.

**Design decision 3 — a new file, `tests/test_decklink_live_backsrc_demo.cpp`, not an extension of `tests/test_decklink_live_sphere.cpp`.** That file is 909 lines as of this session (confirmed directly, not assumed from any prior session's account) and already carries a substantial interactive keyboard-control surface (WU-21h/21i/22c/35a2/35a3) this unit has no use for and would only have grown further. `SESSION-PROTOCOL.md`'s own work-unit-sizing rule and its own per-test-file fixture-isolation convention both point the same direction a new file does: this unit's own footprint (one new test file, one `CMakeLists.txt` change, zero `src/` changes) stays comfortably inside the 3-source-file cap either way, but a new file keeps this unit's own real, novel content (the `backSrc` wiring, the automatic sweep, the zone-plate generation) legible on its own rather than interleaved into an already-large file's own pre-existing control-surface code.

**Design decision 4 — no command-line flag, no `kBufferMode`/`frontTag`/`backTag` configuration.** `WORK-UNITS.md`'s own prior WU-33c4 sketch (Session 78) assumed this unit would be "where the actual command-line selection surface lives... a flag choosing a static back-source file versus a Blackmagic sub-device" — not trusted, per that note's own explicit instruction, and confirmed unnecessary this session on two independent grounds. First, with `DeckLinkBackSource` split out to WU-33c5 (decision 1, above), there is only one producer left in this unit's own scope, so there is nothing left to select between. Second, confirmed directly against `core/pipeline.cpp` this session, not assumed: `runFrame()`'s own threads<=1 branch calls `generateFragments(lattice, src, params.maxK, params.supersample, params.tag, bins, shadingGrid, params.backSrc)` — the plain, single-scalar-tag, `kBufferMode == Off` path — and forwards `params.backSrc` into it exactly the same as the `kBufferMode != Off && frontTag != backTag` `TagByFacing` branch does; `core/pipeline.cpp`'s own comment above these call sites states this explicitly ("this file does not gate `backSrc`'s propagation on [tag/kBufferMode configuration]... `backSrc` reaches either sibling equally"). `test_decklink_live_sphere.cpp`'s own `kBufferMode = Blend`/`frontTag = 1`/`backTag = 2` configuration (WU-35a2/35a4) is there to make *k-buffer resolve tags* and *`manualTransp` blending* operator-visible — a separate mechanism from `backSrc` content substitution, confirmed independent by `core/binner.hpp`'s own file comment on `backSrc`. This unit's own default `PipelineParams` (`destWidth`/`destHeight` only) is genuinely enough.

**Design decision 5 — back-side content: a real zone plate (`tools/testpat.hpp`'s own `makeZonePlate()`/`writeV210()`), generated to a temp file at startup, not a solid colour.** Mirrors `tests/test_decklink_output.cpp`'s own `writeWarpedTestFrame()`, which already uses exactly this generate-then-write-then-read-back shape for its own front-side source. Considered and rejected: reusing WU-33c3's own test convention of a plain black frame (`video::packBlackFrame`, ADR-064) — sufficient there because that test's own job was proving the wiring fired at all (`backSourceQueried` moving), not a by-eye content check; this unit's whole point is the opposite, a real by-eye "genuinely different content" claim, and a solid colour risks coincidentally matching whatever a live front signal's own average field happens to be, proving nothing by eye even if the wiring is perfectly correct. A zone plate's own concentric rings cannot be mistaken for a plausible live-capture signal.

**Files:** `tests/test_decklink_live_backsrc_demo.cpp` (new, 363 lines). `CMakeLists.txt` (modified — new `add_executable`/`target_link_libraries`(`scatter-decklink scatter-core`, no `scatter-diag`)/`target_include_directories`(`tests` + `tools`, the latter for `testpat.hpp`)/`add_test` block, inserted immediately after `test_decklink_live_sphere`'s own block). Zero `src/` files touched — this unit is pure composition of already-existing pieces (WU-33c3's `BackSourceCallback`, WU-33c2a's `FileBackSource`, WU-21c's `LiveFramePlayback`, WU-03's `testpat::makeZonePlate()`/`writeV210()`).

**Not built, not run, not confirmed against real hardware, by this or any prior session.** Same DeckLink-linked limitation as WU-33c2b/WU-33c3: no `BLACKMAGIC_SDK_DIR`/Cocoa in the cloud sandbox, no `cmake`/`ninja`/`clang++`/macOS in the device bridge's own Linux VM (reconfirmed directly this session: bare `g++ 11.4.0`, no `DeckLinkAPI.h` anywhere). This session's own cloud-sandbox `git clone` of `origin` was reconfirmed working directly. Steve's own real-terminal `close.sh` run is the first time any of this session's own new file will actually compile.

**`docs/wu-audit-2026-08.md` line 113 re-checked directly this session, not fixed, same standing instruction Sessions 72-81 already followed** — still reads "The real-content gap (single tag per call) was already found and logged as C-020/ADR-062 before this sweep, and closed by WU-28c," still incorrect for the same reason already found (the real-content gap wasn't actually closed until WU-35a4/WU-47). Left unfixed, outside this session's own scope — **now ten sessions running** (72 through 82).

**Untouched, deliberately.** `INVARIANTS.md` (not touched — grepped this session for `backSrc`/`CaptureConsumer`/`BackSource`, no match, no invariant this session's own work bears on). `ADR-059/062/065/069/070/072/073/074/077/082/083/084/085/086/087/088/089/090/091/092/093/094/095/096/097` (not reopened; this session's own work is a new ADR, 098). `CORRECTIONS.md` (no new entry — nothing wrong was found in the repository this session; WU-33c3's own already-green real-terminal test run is new information, not a correction of an error). `src/io/decklink_capture_consumer.hpp`/`.cpp`, `src/io/decklink_back_source.hpp`/`.cpp`, `src/io/file_back_source.hpp`/`.cpp`, `src/core/binner.hpp`, `src/core/pipeline.cpp` (all read in full or in part to confirm this unit's own design — not edited). `tests/test_decklink_live_sphere.cpp` (read in full to confirm its own current size/scope and geometry — not edited). `docs/wu-audit-2026-08.md` (flagged above, not fixed).


**ADR-099 — WU-33c4 amended, same session: interactive keyboard controls ported from `test_decklink_live_sphere.cpp`, and `kBufferMode = Blend` added so T/t has a real effect.**

Same-session follow-up to ADR-098, not a new session. After Steve's first real-terminal run of `tests/test_decklink_live_backsrc_demo.cpp` (ADR-098), he raised two things: the device-selection question (parked, see below) and a frame-rate regression he attributed to "the front/back processing" (also parked — see below), and then gave one explicit, actionable instruction: add the keyboard controls `tests/test_decklink_live_sphere.cpp` already has into this new demo file.

**Both parked items, named here so they are not lost, not resolved:** (1) which of Steve's multiple enumerated capture-capable DeckLink devices is actually fed is a real, now-confirmed-to-exist problem (`firstFormatDetectionCapableInput()`'s own "take the first match" convention, duplicated identically across every DeckLink test in this repository including this one, silently breaks once more than one such device exists and the enumeration order does not match which one is cabled) — Steve asked to park this, so nothing in this session changes device selection; this answers WU-33c5's own previously-open hardware question (yes, more than one capture-capable device exists on his machine) without yet fixing the selection bug itself. (2) The frame-rate drop Steve reported "since front/back processing was added" was not narrowed down before this session's own edits (which test, which stat, whether it persists once capture points at the right device) — not investigated or fixed this session; one plausible, not-yet-confirmed candidate worth naming for whoever picks this up: `FileBackSource::currentSourceRaster()` (WU-33c2a) returns a full copy of its own owned frame by value on every call, and `CaptureConsumer::processOne()` calls it once per processed frame (WU-33c3) — a real, already-documented "deliberate, named cost" (`file_back_source.hpp`'s own header comment) that was accepted in the abstract before any session had a real frame rate to measure it against.

**The actual instruction: port `test_decklink_live_sphere.cpp`'s own flag-off (non-`CoverageWindow`) interactive control loop into `test_decklink_live_backsrc_demo.cpp`, replacing the automatic sweep as the primary path.** Ported unchanged: the `Key` enum, `readKey()` (blocking, one logical keypress including escape-sequence arrows), and the flag-off loop's own inline `switch`/`applyKey`-equivalent state updates (cursor keys for yaw/pitch, X/x/Y/y for position, Z/z for radius, T/t for `manualTransp`, Q to quit), duplicated locally per `SESSION-PROTOCOL.md` rule 2 exactly as ADR-098 already established for this file's own lattice functions. Deliberately **not** ported: `test_decklink_live_sphere.cpp`'s own `CoverageWindow`/`--show-coverage` machinery (WU-22c) — a materially separate feature (a Cocoa window, GCD dispatch sources) nobody asked for here, and porting it unasked would itself be the kind of silent scope expansion this project's conventions warn against. The original automatic 30-second yaw sweep (ADR-098) is kept, not deleted — demoted from the primary path to the non-interactive-terminal fallback, the same role `test_decklink_live_sphere.cpp`'s own bounded-sleep fallback plays there, except this file's fallback is a real sweep rather than an idle wait, which is strictly more observable for an unattended run and was already written.

**A real design addition, not asked for by name but load-bearing for the ask to work at all: `params.kBufferMode = scatter::KBufferResolveMode::Blend`, plus a live `manualTransp` variable seeded at 0 and pushed to the consumer via `setManualTransp()` on every T/t keypress.** Without this, porting T/t alone would compile and run but change a variable with zero visible effect: confirmed directly against `core/resolve.hpp` this session, not assumed, that `compositeKBuffer()`'s own Blend fold — the mechanism that makes `manualTransp` visible at all — only runs when `params.kBufferMode != KBufferResolveMode::Off`, and that this gate is independent of `frontTag`/`backTag` (unlike the separate `kBufferMode != Off && frontTag != backTag` gate that selects `TagByFacing` fragment generation, confirmed in ADR-098). `frontTag`/`backTag` are therefore left at their `PipelineParams` default (both 0) — this unit's own `backSrc` substitution needs no facing-tag differentiation, only the Blend resolve itself.

**Files:** `tests/test_decklink_live_backsrc_demo.cpp` only (rewritten in place — same file ADR-098 created this session, not a new one; 489 lines). No `CMakeLists.txt` change (nothing about the target's own link libraries or include paths changed — `<termios.h>`/`<unistd.h>` are both provided by the platform C library already reachable from every other DeckLink test in this repository that already uses them, `test_decklink_live_sphere.cpp` included). `WORK-UNITS.md`'s own WU-33c4 entry amended in place to reflect the ported control surface and the two parked items; still `todo`, still device-bridge hand-off only.

**Not built, not run, not confirmed against real hardware, by this or any prior session** — same DeckLink-linked limitation as every prior entry in this file for this file.


**ADR-100 — WU-33c6 scoped: copy-on-write storage for `OwnedSourceRaster`, removing `FileBackSource`'s own per-frame full-buffer copy; ADR-094 extended, not reopened.**

Same-session follow-up to ADR-098/ADR-099, not a new session. Steve asked for a work unit to optimise the current code's own processing speed. Re-derived against the real current code before writing any `Files:`/`Accept:`, per this whole session's own standing instruction not to invent a scope speculatively: rather than write a vague, unscoped "optimise everything" unit, this session went back to the one concrete, already-measured lead ADR-099 itself named and parked — Steve's own real `build/Testing/Temporary/LastTest.log` run of `test_decklink_live_backsrc_demo` (Aug 31, 12:53 BST) shows `framesArrived=752` against only `framesPushed=114` into the 8-slot `CaptureFrameRing` (`io/decklink_input.hpp`'s own `kCaptureRingCapacity=8`) — roughly 15% — and investigated it in full rather than merely re-naming it a second time.

**Confirmed directly this session, not assumed: `FileBackSource::currentSourceRaster()` (`io/file_back_source.cpp`, WU-33c2a) is a real, measurable, unconditional per-frame cost.** Its own header comment already names this in the abstract ("the deliberate, named cost of one avoidable buffer copy per query for this producer's own unchanging case") — not previously sized. `OwnedSourceRaster`'s own `rgb` member (`core/resolve.hpp`, WU-33c1/ADR-094) is a `video::RasterRGB`: three `std::vector<Sample>` (`Sample = std::uint16_t`, `core/types.hpp`), each `width * height` elements. At 720x576 (`bmdModePAL`, this project's own display mode throughout the WU-33c lineage) that is three fresh heap allocations and roughly 2.5 MB copied, on every single `currentSourceRaster()` call — and `CaptureConsumer::processOne()` (WU-33c3, `io/decklink_capture_consumer.cpp`) calls it exactly once per processed frame, unconditionally, whenever a `BackSourceCallback` is wired — for content that, for this producer specifically, never changes after `create()`.

**Confirmed directly this session, by grep across `src/` and `tests/`, not assumed: no code anywhere mutates an `OwnedSourceRaster` after construction.** The only write to any `OwnedSourceRaster`'s own `rgb` member is inside `unpackSourceRaster()` itself (`core/pipeline.cpp`), which fills `out.rgb.R/G/B.data()` via `chroma::ycbcrToRgbImage()` before returning `out` — one write, at construction, before the object exists anywhere else. Every other reference is read-only: `OwnedSourceRaster::view()` returns a `SourceRaster` (`core/binner.hpp`) whose own `r`/`g`/`b` fields are `const Sample*`, and no non-const `view()` overload exists anywhere in this codebase. `FileBackSource::currentSourceRaster()`, `DeckLinkBackSource::currentSourceRaster()`, and `CaptureConsumer::processOne()`'s own `backSnapshot` all only read through `view()`. This is exactly the precondition that makes shared, reference-counted storage safe with no copy-on-write branch, mutex, or defensive clone needed at all: an `OwnedSourceRaster`, once built, is logically immutable for the rest of its life on every code path that exists today.

**Design decision — `OwnedSourceRaster`'s own `rgb` member becomes a `std::shared_ptr<video::RasterRGB>`, not a `video::RasterRGB` owned by value.** Copying an `OwnedSourceRaster` becomes an atomic refcount increment instead of a three-vector deep copy. Considered and rejected: (a) changing the shared `BackSourceCallback`/`std::function<std::optional<OwnedSourceRaster>()>` shape itself to an out-parameter or reusable-buffer convention, avoiding the value-type copy at the interface level instead of inside the type — rejected because it touches the callback signature ADR-095 fixed and every one of its three current implementations (`FileBackSource`, `DeckLinkBackSource`, `CaptureConsumer`), a materially larger and riskier surface than this unit's own 2-file footprint, for no benefit this narrower fix doesn't already capture; (b) a virtual/type-erased "cheap view" interface alongside the existing value type — rejected for the same reason ADR-095 already rejected a virtual interface for pluggability generally (no existing precedent in this codebase outside the Blackmagic SDK's own COM interfaces, which this project has already decided not to imitate for its own internal seams); (c) leaving `OwnedSourceRaster` untouched and instead changing only `FileBackSource`'s own internal caching strategy — rejected because it is not actually possible: `currentSourceRaster()` returns `OwnedSourceRaster` by value, a concrete type whose own copy constructor deep-copies three vectors regardless of what `FileBackSource` does internally before that return; the cost lives in the type being returned, not in how `FileBackSource` stores its own source frame, so only changing the type itself can remove it.

**This is the first use of `std::shared_ptr` anywhere in `src/` — confirmed by grep, named plainly, not presented as an existing pattern being merely extended.** `ComPtr<T>` (`io/com_ptr.hpp`) is this codebase's own existing reference-counted-pointer precedent, but it wraps the Blackmagic SDK's own COM `AddRef`/`Release` convention specifically, not a general-purpose pattern for plain data; it was not reused or adapted here, a `std::shared_ptr<video::RasterRGB>` needing none of `ComPtr`'s own COM-specific machinery.

**Extends `DECISIONS.md` ADR-094, does not reopen it.** ADR-094 settled `OwnedSourceRaster`'s existence, its purpose (a caller-owned, value-semantic snapshot `unpackSourceRaster()` can return by value, replacing an unreachable local `runFrameBytes()`/`runFrameBytesDeinterlaced()` already computed), and its external contract (`view()` returning a `SourceRaster` of raw pointers, matching `SourceRaster`'s own existing shape exactly). None of that changes here: `OwnedSourceRaster` is still a value type, still owns its own storage, still returned by value from `unpackSourceRaster()`, still exposes the identical `view()` signature and behaviour. What ADR-094 did not settle, because no session before this one had a real frame rate to weigh it against (the same gap ADR-099 itself named and parked): the internal cost of copying one. This unit answers that question additively, the same "reused... extended... not modified" shape ADR-044/056's own non-owning-pointer convention has already been given more than once elsewhere in this project's own history, not a revision of ADR-094's own text or claims.

**Files:** `src/core/resolve.hpp` (modified — `OwnedSourceRaster`'s `rgb` member type and constructor, `view()`'s three-line body). `src/core/pipeline.cpp` (modified — `unpackSourceRaster()`'s own five touched lines, `out.rgb.R/G/B.data()`/`out.rgb.planeR()` become `out.rgb->...`; mechanical, no logic change). `tests/test_unpack_source_raster.cpp` (modified, doesn't count against the cap — extends WU-33c1's own existing two checks with two more). Zero files touched in `io/` — confirmed by grep this session: `FileBackSource`, `DeckLinkBackSource`, and `CaptureConsumer` all already treat `OwnedSourceRaster` as an opaque returned value; every one of them gets this unit's own benefit automatically, at every existing call site, with no call-site change anywhere. Comfortably inside the 3-source-file cap (2 of 3 used) — no split needed.

**Accept — written this session, NONE of it built or run:** `tests/test_unpack_source_raster.cpp`'s own two existing checks (WU-33c1) stay green, unchanged in intent — bit-identical output vs. `runFrameBytes()`; a copy's own `view()` stays correct after the original is destroyed (now trivially true, since destroying one `shared_ptr` owner while a copy is alive frees nothing). Two new checks added: (3) after copying an `OwnedSourceRaster`, the copy's own `view().r`/`.g`/`.b` compare pointer-*equal*, not merely value-equal, to the original's own — the direct, sandbox-checkable proof that copying no longer duplicates the underlying buffer; (4) two separately-constructed `OwnedSourceRaster`s (two independent `unpackSourceRaster()` calls, not copies of each other) compare pointer-*unequal* — rules out a broken implementation that shares one static buffer across every instance, which would pass check 3 vacuously.

**Explicitly not claimed: any frame-rate or timing number, from this sandbox or the device bridge's own Linux VM, for either FileBackSource's old or new cost.** Same discipline `DECISIONS.md` ADR-044 already established for WU-19b, for the identical category of reason: a wall-clock measurement against a real hardware target — there, 576i25's own 40 ms/frame budget on Steve's M1 Max; here, whatever margin Steve's own UltraStudio capture pipeline actually needs before `CaptureFrameRing` overflows — means nothing measured on a CPU with no relationship to that target, this session's cloud sandbox and the device bridge's own Linux VM both included. What this unit can and does prove, entirely in-repo: the copy this session's own investigation found and sized is real, and after this change it is gone — not "faster," gone, down from three heap allocations and roughly 2.5 MB memcpy'd to one atomic increment, per call, unconditionally. **Whether that was actually the cause of Steve's own reported frame-rate drop, and whether `framesPushed`/`framesArrived`'s own ratio improves on his own real terminal, is his own job to confirm** by rebuilding, rerunning `test_decklink_live_backsrc_demo`, and comparing the ratio this ADR's own evidence section already cites — not asserted here. If the ratio doesn't move, the real cause is still open, and this unit's own value stands on its own correctness/design merits regardless: a real, needless per-frame cost, named in this codebase's own comments since WU-33c2a, is actually gone either way.

**`docs/wu-audit-2026-08.md` line 113: not re-checked this ADR** — already re-confirmed stale twice this same session (ADR-097, ADR-098); nothing about this unit's own scope bears on it, and this project's own established convention (ADR-099 already set the precedent) is to re-state it on a fresh session-open verification, not on every same-session follow-up ADR.

**Untouched, deliberately.** `INVARIANTS.md` (not touched — grepped this session for `OwnedSourceRaster`/`RasterRGB`/`copy`, no match, no invariant this unit's own work bears on). `CORRECTIONS.md` (no new entry — nothing wrong was found in the repository this session; the copy cost was already honestly documented by WU-33c2a's own header comment, not a bug, and grepped clean, not assumed, for any past note about `shared_ptr`/`OwnedSourceRaster` aliasing). `ADR-059/062/065/069/070/072/073/074/077/082` through `097` (not reopened — ADR-094 in particular extended, per the account above, not revised; this session's own work is a new ADR, 100). `video::RasterRGB` itself (`video/raster.hpp`, untouched — used elsewhere as genuine local scratch storage, `runFrameBytes()`/`runFrameBytesDeinterlaced()`'s own locals, confirmed by grep structurally unrelated to `OwnedSourceRaster::rgb`). `core/binner.hpp`/`SourceRaster` (already read-only, `const Sample*`, needs no change). `io/file_back_source.hpp`/`.cpp`, `io/decklink_back_source.hpp`/`.cpp`, `io/decklink_capture_consumer.hpp`/`.cpp` (WU-33c2a/WU-33c2b/WU-33c3, read to confirm zero-touch, not edited). The device-selection question and the frame-rate claim's own final real-hardware confirmation (both still open, both explicitly Steve's own job, not this unit's).


**ADR-101 — WU-33c6 built and tested: `OwnedSourceRaster::rgb` is now a `std::shared_ptr<video::RasterRGB>`, confirmed green in three sandbox configurations plus an independent real-hardware compiler; ADR-100 not revised.**

Same-session follow-up to ADR-100 (Session 82), this time a new session (Session 83) picking up WU-33c6 where ADR-100 left it: scoped, not built. Opened by verifying the real repository state directly rather than trusting the incoming prompt's account of it — `HEAD` and `origin/main` both `aac353c` ("WU-33c6 scoped (not built)"), working tree dirty only in `HANDOFF.md` (Session 82's own draft, never itself a blocker per `SESSION-PROTOCOL.md`), matching expectations. Re-read `WORK-UNITS.md`'s own WU-33c6 entry and this ADR-100 in full, then re-derived the scope directly against `src/core/resolve.hpp` and `src/core/pipeline.cpp` as they exist right now, per this session's own explicit instruction not to assume the prior session's scoping was still accurate without checking.

**One discrepancy found by that re-derivation, not a scope error: ADR-100/WU-33c6's own `Files:` line said `core/pipeline.cpp`'s own `unpackSourceRaster()` had "five touched lines".** Direct read found exactly two physical lines reference `out.rgb` inside that function — `out.rgb.R.data(), out.rgb.G.data(), out.rgb.B.data(),` and `out.rgb.planeR().strideSamples);` — four `.` → `->` conversions across those two lines, not five lines. Grepped `core/pipeline.cpp` for every other `.rgb`/`rgb.` occurrence to be sure nothing else needed touching: the other nine hits (lines documented in `WORK-UNITS.md`'s own WU-33c6 entry) all belong to local `video::RasterRGB rgb;` variables inside `runFrameBytes()`/`runFrameBytesDeinterlaced()`/`runFrameFile()` — genuine local scratch storage, structurally unrelated to `OwnedSourceRaster::rgb`, confirmed unaffected and left untouched, exactly as ADR-100's own "untouched, deliberately" section already said `video::RasterRGB` itself would be. Not logged as a `CORRECTIONS.md` entry: a descriptive miscount in the prior session's own prose, not a wrong `Files:`/`Accept:` line (the two actual files were the two named), not a code or test defect — the same threshold `docs/wu-audit-2026-08.md`'s own WU-26 line already used for a comparable cosmetic finding (a stale comment, corrected in that document's own table without a `CORRECTIONS.md` entry).

**Built exactly as ADR-100 designed, no further design changes.** `src/core/resolve.hpp`: `OwnedSourceRaster::rgb` changed from `video::RasterRGB` (owned by value) to `std::shared_ptr<video::RasterRGB>` — the first `std::shared_ptr` anywhere in `src/`, confirmed by grep both before and after this change (the only pre-existing reference-counted-pointer precedent, `ComPtr<T>` in `io/com_ptr.hpp`, wraps the Blackmagic SDK's own COM convention specifically and was not reused, exactly as ADR-100 already noted). Constructor changed to `rgb(std::make_shared<video::RasterRGB>(width, height))`; `view()`'s five-field body changed from `rgb.width`/`rgb.height`/`rgb.R.data()`/`rgb.G.data()`/`rgb.B.data()` to the `rgb->`-prefixed equivalents. Added `#include <memory>`. The struct's own header comment was extended (new paragraph appended, original text unchanged) to record the design and its safety precondition, rather than rewritten. `src/core/pipeline.cpp`: the two `out.rgb.` lines identified above became `out.rgb->`, nothing else in the file touched. `tests/test_unpack_source_raster.cpp`: the file's own header comment's "Two checks" list extended to "Four checks" with checks 3/4 described; check 2's own two comments that were rendered stale by this change ("deep copy: RasterRGB's own std::vector members reallocate" and "original's own storage is genuinely freed here") were corrected in place to describe the new aliasing-copy/shared-ownership behaviour, rather than left standing as now-false claims about the code beside them — a direct application of this project's own repeated precedent of rewriting a comment a change has made false (most recently `core/pipeline.cpp`'s own header comment after WU-47, `HANDOFF.md` Session 74) rather than leaving it to mislead the next reader. Two new test functions added: `test_owned_source_raster_copy_shares_storage_pointer_equal()` (check 3) and `test_owned_source_raster_independent_instances_do_not_share_storage()` (check 4), both wired into `main()`. No other file touched; no `CMakeLists.txt` change needed (the test executable was already registered by WU-33c1).

**Built and run for real, in the cloud sandbox and on the device bridge's own Linux VM — full results:**

1. Baseline, before any edit: cloned `origin` fresh (`git clone https://github.com/skunge2000/scatter-dve.git`) into the cloud sandbox, confirmed `HEAD` at `aac353c`, configured with CMake/Ninja (`-DCMAKE_BUILD_TYPE=Release`, GCC 13.3.0, no `BLACKMAGIC_SDK_DIR`), built `test_unpack_source_raster`, ran it: 7/7 checks passing, matching WU-33c1's own already-`green` baseline exactly.
2. Applied the three edits above to the cloned working copy. Rebuilt the entire `scatter-core` target and every portable test executable (70 build steps, zero errors, zero warnings) and ran the full `ctest` suite: **32/32 green**, GCC 13.3.0 Release. `test_unpack_source_raster` itself: 13 checks (up from 7 — six new `CHECK` calls across the two new test functions).
3. Reconfigured and rebuilt with `CMAKE_CXX_COMPILER=clang++` (Clang 18.1.3), Release: full 70-step build clean, full `ctest` suite **32/32 green**.
4. Reconfigured and rebuilt with `CMAKE_BUILD_TYPE=Debug` and `CMAKE_CXX_FLAGS`/`CMAKE_EXE_LINKER_FLAGS` = `-fsanitize=address,undefined -fno-sanitize-recover=all` (GCC 13.3.0): full 70-step build clean, full `ctest` suite **32/32 green**; `nm -D` on `test_unpack_source_raster` found 42 `asan`/`ubsan` symbol references, `ldd` confirmed `libasan.so.8`/`libubsan.so.1` both genuinely linked — the same instrumentation-is-real check every prior sandbox-verified WU-33c-lineage unit has used.
5. **Regression-detection proof, the same discipline WU-33c1/WU-33c2a's own sessions already used, run twice for this unit's own two new checks specifically:** (a) temporarily reverted `OwnedSourceRaster::rgb` to an owned-by-value `video::RasterRGB` (the pre-this-unit type) in the sandbox working copy only, rebuilt, and confirmed exactly check 3's own three `CHECK`s failed (`copyView.r/g/b == originalView.r/g/b`) and nothing else — 10 of 13 checks still passing; (b) reverted that mutation (confirmed byte-identical to the pre-mutation file by `diff`, rebuilt to 13/13 green), then separately mutated the constructor to cache one `static std::shared_ptr` shared across every `OwnedSourceRaster` instance (the "broken single-shared-static-buffer" failure mode check 4 exists to catch) and confirmed exactly check 4's own three `CHECK`s failed (`aView.r/g/b != bView.r/g/b`), check 3 correctly unaffected — reverted again, confirmed byte-identical, rebuilt to 13/13 green. Both mutations and reversions were made only in the cloud sandbox's own disposable clone, never written to the real repository.
6. All three changed files (`src/core/resolve.hpp`, `src/core/pipeline.cpp`, `tests/test_unpack_source_raster.cpp`) written into the real repository (`~/src/scatter-dve`) via the device bridge, then re-staged from the device and diffed byte-for-byte against the cloud sandbox's own confirmed-green working copy — identical, all three files.
7. Independent confirmation beyond the diff: the real repository's own just-written files were compiled directly on the device bridge's own Linux VM, which has no CMake/Ninja/Clang (confirmed again this session, matching every prior session's own finding) but does have a third, independent compiler, `g++ 11.4.0`. Compiled by hand (no CMake) against this project's own exact `-std=c++20 -Wall -Wextra -Wpedantic -Wconversion -Wsign-conversion -Werror` flag set: `test_unpack_source_raster` — 13/13 green, zero warnings; `test_file_back_source` (WU-33c2a, unchanged) — 8/8 green; `test_pipeline_backsrc` (WU-33a/b, unchanged) — 17/17 green. All three built and run directly from the real, just-written repository files, not the sandbox copy. Scratch object files and binaries this check produced were written to this device-bridge session's own `$HOME` (outside the mounted `mnt/` folder, per this session's own operating constraints) and deleted before session end; none were ever committed or left in the repository.

**Extends `DECISIONS.md` ADR-094 and ADR-100, revises neither.** ADR-094's own settled text (`OwnedSourceRaster`'s existence, purpose, `view()` contract) is untouched. ADR-100's own design decision (`std::shared_ptr<video::RasterRGB>`, the three rejected alternatives, the "first `std::shared_ptr` in `src/`" finding, the explicit non-claim about frame-rate numbers) is exactly what got built — this entry documents the build and test confirmation ADR-100 itself said had not yet happened, not a different design.

**`docs/wu-audit-2026-08.md` line 113: re-checked directly this session, still stale — eleven-plus sessions running now (72 through 83).** Still reads "The real-content gap (single tag per call) was already found and logged as C-020/ADR-062 before this sweep, and closed by WU-28c. Nothing new," which is still wrong for the same reason Sessions 72-82 already found: `WU-28c` only built the `TagByFacing` functions, and the whole-frame/row-range gap was not actually closed until `WU-35a4` (frame mode) and `WU-47` (field mode). Left unfixed — outside this unit's own `Files:` scope (`src/core/resolve.hpp`, `src/core/pipeline.cpp`, `tests/test_unpack_source_raster.cpp`), same restraint every session since 72 has already applied.

**Files:** `src/core/resolve.hpp`, `src/core/pipeline.cpp`, `tests/test_unpack_source_raster.cpp` — exactly the three ADR-100/WU-33c6 already named. No new files, no `CMakeLists.txt` change.

**Untouched, deliberately.** `INVARIANTS.md` (not touched — grepped again this session for `OwnedSourceRaster`/`RasterRGB`/`shared_ptr`, no match, no invariant this unit's work bears on). `CORRECTIONS.md` (no new entry — the one discrepancy this session found, discussed above, was judged cosmetic, the same threshold this project's own `docs/wu-audit-2026-08.md` already set for a comparable finding, not a `Files:`/`Accept:`/code/test error). `ADR-059/062/065/069/070/072/073/074/077/082` through `099` (not reopened — ADR-094 extended per ADR-100's own account, ADR-100 itself extended per this entry, neither revised). `io/file_back_source.hpp`/`.cpp`, `io/decklink_back_source.hpp`/`.cpp`, `io/decklink_capture_consumer.hpp`/`.cpp` (re-confirmed zero-touch by grep this session, not edited — every current `OwnedSourceRaster` caller already treats it as an opaque returned/copied value). `video::RasterRGB` itself (`video/raster.hpp`, untouched — still genuine local scratch storage elsewhere, confirmed structurally unrelated to `OwnedSourceRaster::rgb` by this session's own line-by-line grep review above). `core/binner.hpp`/`SourceRaster` (already read-only, `const Sample*`, needs no change). The frame-rate claim's own final real-hardware confirmation and the device-selection question (both still open, both explicitly Steve's own job, unaffected by this unit actually being built).

**Not committed, tagged, or pushed by this session, per this project's own standing instruction** — Steve's own exact commands are in `HANDOFF.md`.
